                    ...
                    @foreach ($rows as $index => $row)
                        {{-- <div class="row">
                            <label for="formFileMultiple2"
                                class="btn btn-primary btn-sm form-label mx-5 mt-3 p-2 w-100 radius-10 text-center"><span
                                    class="fa fa-image mx-2"></span> ({{ $index + 1 }} ) اختر صورة للمنصة</label>
                            <input class="form-control" type="file" wire:model="rows.{{ $index }}.image"
                                id="formFileMultiple2" hidden wire:loading.attr="disabled"
                                wire:target='rows.{{ $index }}.image' />

                            <div wire:loading wire:target="rows.{{ $index }}.image">تحميل</div>
                            <div class="progress">
                                <div class="progress-bar" style="width:0%">0%</div>
                            </div>
                            <div class="cart-img-container">
                                ...
                                @if (is_string($row['image']))
                                    <div class="deleter" wire:click="deleteImagePlatform({{ $index }})">
                                        <span class="fa fa-trash "></span>
                                    </div>
                                    <img src="{{ asset('storage/' . ltrim($row['image'], '/')) }}" alt="Image"
                                        class="img-thumbnail mb-2" width="100">
                                @elseif ($row['image'])
                                    <div class="deleter" wire:click="deleteImagePlatform({{ $index }})">
                                        <span class="fa fa-trash "></span>
                                    </div>
                                    <img src="{{ $row['image']->temporaryUrl() }}" class="img-thumbnail mb-2"
                                        width="100">
                                @endif
